...
bool Search(vector<int> v, int n, int x){
    ...
    while(left <= right){                       // so we'll first find the sorted part then check for target in that part
        ...
        if(v[left] == v[right]){   
            ...
            left++;                             // For the cases when we'll not be able to identify the sorted part
            right--;                            // so we'll shrink the search space
            continue;
        }

        if(v[left] <= v[mid]){                  // Checking if left part is sorted
...
int main(){
    ...
    int n;
    cin>>n;
    vector<int> v(n);                           // The array contains duplicate elements in sorted fashion
